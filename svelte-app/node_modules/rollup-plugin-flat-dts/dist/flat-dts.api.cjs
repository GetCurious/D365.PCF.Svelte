...
class DtsSetup {
    ...
    sourceURL(path) {
        return new node_url.URL(path, this.root());
    }
    root() {
    ...
    relativePath(path) {
        ...
        const { href } = new node_url.URL(path, cwd);
...
class DtsSourceMap {
    static async create(path, content, setup) {
        return new DtsSourceMap(await new sourceMap.SourceMapConsumer(content, setup.sourceURL(path).href), setup);
    }
...
function createFlatDts(files, diagnostics = []) {
    return {
        ...
        formatDiagnostics,
        writeOut(rootDir) {
            const filePath = rootDir != null ? ({ path: path$1 }) => path.resolve(rootDir, path$1) : ({ path }) => path;
...
class DtsPrinter {
    ...
    createFile(path$1, content = this._out) {
        return {
            path: path$1,
            content,
            async writeOut(filePath = path$1) {
                await node_fs.promises.mkdir(path.dirname(filePath), { recursive: true });
                return node_fs.promises.writeFile(filePath, content);
            },
        };
    }
}
...
async function packageName() {
    const packageJson = await node_fs.promises.readFile('package.json', { encoding: 'utf-8' });
    ...
    _emitFiles(statements, diagnostics) {
        ...
        for (const { to: topLevel, dia, refs } of statements) {
            ...
            for (const [info, statement] of topLevel) {
                if (info.file) {
                    const key = path.resolve(info.file);
...
exports.emitFlatDts = emitFlatDts;
//# sourceMappingURL=flat-dts.api.cjs.map
